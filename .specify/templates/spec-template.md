...
**Why this priority**: [Explain the value and why this has this priority level]
...
- **FR-006**: 規格 MUST 說明是否影響共用 `Layout`、`Header`、`Menu`、`Content`、
  `Footer` 或文章呈現元件
- **FR-007**: 規格 MUST 說明內容來源是 Markdown、資料檔或既有靜態內容，並標示
  是否需要內容遷移
- **FR-008**: 規格 MUST 說明功能是否仍可輸出為 GitHub Pages 可部署的靜態檔案
...
- **Layout Impact**: [說明是否變更共用 Layout 或導覽結構]
- **Content Source**: [說明內容來自 Markdown、資料檔或既有靜態頁面]
- **Reusable Components**: [列出可沿用或需新增的共用元件]
- **Static Deployment Check**: [說明為何仍符合 GitHub Pages 靜態部署限制]
